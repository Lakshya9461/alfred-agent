...
| 26 | `/skills` appeared **stuck on "Refreshing skill repos..."** — the handler awaited `ensure_repos()` (git clone/pull, per-repo timeouts up to 300s) for 180s; on hosts where HTTPS git hangs on a credential prompt (SYSTEM service has no TTY) the reply never came for minutes | `/skills` now replies **instantly from the in-memory index cache** and kicks `ensure_repos()` to a background thread (fire-and-forget) — it never blocks the reply. Hardened all git subprocess calls in `skills.py` (`_git` + clone): `stdin=DEVNULL`, `GIT_TERMINAL_PROMPT=0`, `GIT_ASKPASS=""`, batch-mode SSH, so a missing credential fails fast instead of hanging. Clone timeout lowered 300→120s |
| 27 | `/skills` showed an **empty index despite 985 SKILL.md files on disk**, and once repos were populated the reply **disappeared entirely** | Two separate causes. (a) The service process cached the index on first scan — before the repos were cloned on that host — and never invalidated it (a hung `ensure_repos` dies before `invalidate_cache()`), so `/skills` kept reporting "(no skills installed yet)". Fix: `load_skills(force=True)` rescans from disk on every `/skills` invocation. (b) With all repos populated the single reply exceeded Telegram's **4096-char message limit**, so `reply_text` raised and the handler died silently — no output at all. Fix: `format_skill_index()` now caps lines (60) and total size (3500 chars) with a "… and N more" trailer; the same cap also shrinks the system-prompt skill index from ~112KB to ~8KB per turn |
| 28 | Skill index showed **duplicate entries — one per language** (ECC ships each skill as en/ja/zh SKILL.md files), inflating `/skills` from ~345 real skills to ~950 lines and making `read_skill` return the Japanese copy | `_scan()` now dedupes by `(repo, name)` keeping the description with the highest ASCII-letter ratio (the English copy). `/skills` on dev: 863→345 unique skills |
| — | **Feature: skill self-curation + search** | (1) **`search_skills(keyword)` tool** — the system-prompt index is capped at the 60 most-used skills (it must fit Telegram's 4096-char limit), so the model can now discover *any* of ~345 skills by keyword, then `read_skill` the exact one. (2) **Usage tracking** — every `read_skill` records `{uses, last_used}` in `data/skills_usage.json`; the index orders by usage so the visible 60 are the most relevant. (3) **Auto-prune** — `monitor.skill_curation` disables skills that were used but went stale (`SKILLS_PRUNE_AFTER_DAYS`, default 14); pruned skills vanish from the index but `read_skill` on one re-enables it on the spot. Never-used skills are never pruned (first-run safety). (4) **Auto-absorb** — skills used `SKILLS_ABSORB_MIN_USES` (default 2)+ times are copied into `data/skills_absorbed.json` + a summary lesson, so Alfred keeps the procedure as his own knowledge even if the skill is pruned or its repo is removed. Notifications are plain text (Markdown-unsafe names). |
...
| `SKILLS_PRUNE_AFTER_DAYS` | — | `14` | Auto-disable a skill not used in this many days (used-then-stale only; never-used skills are left alone). `0` = never prune |
| `SKILLS_ABSORB_MIN_USES` | — | `2` | Copy a skill into Alfred's own knowledge (`data/skills_absorbed.json` + lesson) after this many uses. `0` = never absorb |
| `SKILLS_CURATE_INTERVAL` | — | `21600` | Seconds between prune/absorb passes (6h) |
